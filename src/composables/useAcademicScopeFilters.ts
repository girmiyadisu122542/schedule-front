...
/**
 * The academic scope every screen with an academic filter bar reads through:
 ...
 */
export function academicScopeFiltersManager() {
    ...
    /**
     * Axes that narrow a different dimension entirely, so they neither clear
     * the cascade nor are cleared by it. Only the screens that use them set
     * them; the rest leave them null and they never reach the wire.
     */
    ...
    const courseDropdown = useDropdownOptions<DropdownOption>('/courses', FILTER_PARAMS);
    const instructorDropdown = useDropdownOptions<DropdownOption>('/instructors', FILTER_PARAMS);
    ...
    /**
     * Courses and instructors hang off the DEPARTMENT, not the programme —
     * `courses.department_id` and `instructors.department_id` are where
     * ownership lives. So they reload when the department changes, and a
     * programme or section choice leaves them alone.
     */
    watch(departmentId, (id) => {
        courseId.value = null;
        instructorId.value = null;
        courseDropdown.fetchOptions(true, id ? { department_id: id } : {});
        instructorDropdown.fetchOptions(true, id ? { department_id: id } : {});
    });
    ...
    const params = computed<Record<string, number | string>>(() => {
        ...
        // Separate axes, so they stack with whatever the cascade produced
        // rather than replacing it.
        if (examTypeCode.value) applied.exam_type_code = examTypeCode.value;
        if (courseId.value) applied.course_id = courseId.value;
        if (instructorId.value) applied.instructor_id = instructorId.value;
        if (semesterId.value) applied.semester_id = semesterId.value;
        if (statusCode.value) applied.status_code = statusCode.value;
    ...
    const clear = () => {
        ...
        programId.value = null;
        sectionId.value = null;
        // Their own axes, so they have to be cleared explicitly — no watcher
        // above them will do it.
    ...
    const load = () => {
        ...
        courseDropdown.fetchOptions(true, departmentId.value ? { department_id: departmentId.value } : {});
        instructorDropdown.fetchOptions(true, departmentId.value ? { department_id: departmentId.value } : {});
    };
